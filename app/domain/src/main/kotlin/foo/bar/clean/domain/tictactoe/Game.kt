...
interface AutoPlayerService {
    suspend fun getAutoPlayersTurn(): Either<DomainError, NextTurn>
}
...
class Game (
    ...
    private fun fetchAutoPlayerResponse() {
        ...
        updateStateAndNotify(state.copy(isLoading = true, error = DomainError.NO_ERROR))
    ...
    private suspend fun processError(domainError: DomainError) {
        awaitMain { // pop to the UI thread to update the state
            updateStateAndNotify(state.copy(isLoading = false, error = domainError))
        }
    }

    private suspend fun processAutoPlayerMove(nextTurn: NextTurn) {
